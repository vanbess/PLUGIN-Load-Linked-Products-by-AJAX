(function ($) {
    ...
    // append css loader cont to main
    $('#main').append('<div class="sbwc_linked_prods_loading_spinner"></div>');

    // prevent pointer events on #main
    ...
    // log actual start time in current time, i.e. hour now and minute now
    console.log('SBWC AJAX Linked products load start time: ' + new Date().getHours() + ':' + new Date().getMinutes());
    ...
    // Only execute when all AJAX requests have completed
    $.when.apply($, deferreds).done(function () {

        // set endtime
        endtime = new Date().getTime();

        // remove css loader over #main
        $('#main').css('position', 'static');

        // undim main
        $('#main').css('opacity', '1');

        // allow pointer events on #main
        $('#main').css('pointer-events', 'auto');

        // remove css loader cont from main
        $('.sbwc_linked_prods_loading_spinner').remove();
